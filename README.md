...
- 右键菜单可以：
  ...
  - 切换当前后端的模型；Codex 会从实际选中的本地 CLI 读取当前账号可见的模型目录，读不到时只保留默认。若 Codex 运行时仍拒绝所选模型，PRTS 只会清除该模型覆盖、丢弃不兼容的 CLI session，并使用 CLI/config 默认值重试一次。Claude 没有这种目录命令，所以提供一组当前可用的模型（别名 + 版本），若选了账号不可用的型号，会自动切回默认并重试。
  - Claude 与 Codex 后端均可单独选择推理强度。Claude 菜单来自当前本地 CLI 的 `--effort` 能力，模型不支持某档时由 Claude Code 自动降级；Codex 菜单只显示当前模型目录声明支持的档位。选择「默认」会继续沿用对应 CLI/config；若 Codex CLI 仍拒绝所选档位，PRTS 只清除推理强度覆盖并按默认值重试一次。
  - 设置聊天工作目录。
  - 打开数据目录。
  - 退出应用。
...
| 文件 | 用途 |
| --- | --- |
| `settings.json` | 应用设置：当前后端、Claude/Codex 模型及推理强度、聊天工作目录、agent mode、技能（放音乐/搜索/打开网址·应用）、老婆模式（开关 + 间隔/冷却/安静时段/每日上限）、服装、自动截图设置、外观（system / light / dark）、popover 尺寸。 |
...
普瑞赛斯的「**普猫猫**」彩蛋形象由画师 **-浅蓝笑** 原创绘制，经其授权收录于本应用。
低概率彩蛋——每次聊天窗口与桌宠互相切换的瞬间，她都有机会变成这只猫（含「普猫猫」与「普猫猫哭」两种）。
...
出于对原作者的尊重，软件只收录上面这两张原图（正常与哭泣），**不会**交给任何 AI 去生成其他表情或姿势；
日后若要新增猫猫状态，也只会使用画师重新绘制的图。

- 画师主页：**[B站 @-浅蓝笑](https://space.bilibili.com/3493287025445075)**
- 抖音号：`26916156149`（抖音内搜索即可，主页见下图）
- 原作品视频：[bilibili · BV1ZKVY6sESy](https://www.bilibili.com/video/BV1ZKVY6sESy)

<p align="center">
  <img src="docs/credits/douyin-qianlanxiao.jpg" alt="画师抖音主页 · -浅蓝笑（抖音号 26916156149）" width="300">
</p>
...
除上方「致谢」中列明、经画师授权收录的「普猫猫」插画外，这个仓库不内置其他第三方版权美术。
角色图像请在权利方条款和画师授权范围内使用。
