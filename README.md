...
- 右键菜单可以：
  ...
  - 切换当前后端的模型；Codex 会从实际选中的本地 CLI 读取当前账号可见的模型目录，读不到时只保留默认。Claude 没有这种目录命令，所以提供一组当前可用的模型（别名 + 版本），若选了账号不可用的型号，会自动切回默认并重试。
  - Claude 与 Codex 后端均可单独选择推理强度。Claude 菜单来自当前本地 CLI 的 `--effort` 能力，模型不支持某档时由 Claude Code 自动降级；Codex 菜单只显示当前模型目录声明支持的档位。选择「默认」会继续沿用对应 CLI/config。
  - 设置聊天工作目录。
  - 打开数据目录。
  - 退出应用。
